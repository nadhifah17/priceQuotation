    ...
    <body class="hold-transition sidebar-mini layout-fixed @if(isset($current_lang) && $current_lang->dir == 'rtl') rtl @endif">
        ...
        <div class="wrapper">
            ...
            <div class="preloader flex-column justify-content-center align-items-center">
                <img class="animation__shake" src="{{ $model->getFirstMediaUrl('loading_logo') ? $model->getFirstMediaUrl('loading_logo') : ( $system_logo->getFirstMediaUrl('system_logo') ? $system_logo->getFirstMediaUrl('system_logo') : asset('assets/lte/rus.png') ) }}" alt="Logo" height="100">
            </div>
        ...
        <script>
            ...
            function formatPrice(event) {
                ...
                const lastIsComma = input.slice(-1) == ',';
                const commasCount = input.split(',').length - 1;
                const lastIsZero = input.slice(-1) == '0';
                if (lastIsComma) {
                    if (commasCount > 1) {
                        const firstCommaIndex = input.indexOf(',');
                        const first = input.slice(0, firstCommaIndex + 1);
                        const second = input.slice(firstCommaIndex + 1);
                        input = first + second.replace(/[,]/g, '');
                        $this.val(input);
                    }
                    return;
                }
                if (commasCount > 0 && lastIsZero) {
                    return;
                }
                ...
                $this.val(function () {
                    return (input === 0) ? 0 : input.toLocaleString("in-ID", {maximumFractionDigits: 10});
                });
            }

            function transformToOriginalNumber(inputElement) {
                const input = $(inputElement);
                let val = input.val().replace(/[($)\s\._\-]+/g, '').replace(/[,]/g, '.');
                input.val(val);
            }

            function transformToFormattedNumber(inputElement) {
                const input = $(inputElement);
                let inputValue = input.val();
                inputValue = inputValue ? parseFloat(inputValue, 10) : 0;
                input.val(function () {
                    return (inputValue === 0) ? 0 : inputValue.toLocaleString("in-ID", {maximumFractionDigits: 10});
                });
            }

            function formatNumberToTextWithSeparator(number) {
                return (number === 0) ? 0 : number.toLocaleString("in-ID", {maximumFractionDigits: 10});
            }
        </script>
